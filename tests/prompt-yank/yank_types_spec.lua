...
describe('prompt-yank yank types', function()
  ...
  it('yank_context includes surrounding lines', function()
    ...
    local expected = '`ctx.lua#L1-L3` (with 1 lines context)\n```lua\nl1\nl2\nl3\n```'
  ...
  it('yank_blame formats blame output', function()
    ...
    local text = stub(
      git,
      {
        ...
        blame_for_file = function() return 'BLAME' end,
      },
      function()
  ...
  it('yank_tree formats full project tree output', function()
    ...
    local text = stub(tree_mod, {
      build_tree = function() return tree_out, 1 end,
    }, function() return py.yank_tree({ notify = false, register = '"' }) end)
  ...
  it('yank_remote formats remote URL + code output', function()
    ...
    local text = stub(
      ...
      {
        is_git_repo = function() return true end,
        head_sha = function() return 'deadbeef' end,
        remote_url = function() return 'https://github.com/acme/repo.git' end,
      },
      function()
  ...
  it('yank_multi uses picker selections and copies output', function()
    ...
    stub(picker, {
      pick_files = function(_, cb) cb({ 'a.lua', 'b.txt' }) end,
    }, function()
      stub(yank, {
        copy = function(text)
          copied = text
      ...
      }, function() py.yank_multi({ notify = false, register = '"' }) end)
    end)
