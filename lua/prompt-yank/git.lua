...
local function relpath_in_root(fullpath, root)
  ...
  rel = rel:gsub("^%./", "")
  if rel == "." then
    return util.path_basename(fullpath)
  end
  return rel
end
...
function M.blame_for_file(fullpath, root, line_start, line_end)
  ...
  if line_start and line_end then
    table.insert(cmd, ("-L%d,%d"):format(line_start, line_end))
  end
  table.insert(cmd, "--")
...
function M.build_remote_url(base_url, provider, commit, filepath, line_start, line_end)
  if not base_url or not commit or not filepath then
    return nil
  end
