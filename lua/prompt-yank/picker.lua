    ...
    attach_mappings = function(prompt_bufnr)
      actions.select_default:replace(function()
        ...
        for _, entry in ipairs(multi) do
          ...
          if p then
            p = util.trim(p)
            if util.path_is_absolute(p) then
              p = util.path_relative(p, root)
            end
...
local function pick_with_input(root)
  ...
  while true do
    ...
    if util.path_is_absolute(input) then
      input = util.path_relative(input, root)
    end
...
function M.pick_files(opts, on_done)
  ...
  if preferred == "fzf-lua" or preferred == "auto" then
    if pick_with_fzf(root, on_done) then
      return
    end
  end

  if preferred == "telescope" or preferred == "auto" then
    if pick_with_telescope(root, on_done) then
      return
    end
  end
