...
local function should_ignore(path, ignore)
  ...
  for _, dir in ipairs(ignore or {}) do
    ...
    if path:find(dir .. '/', 1, true) == 1 then return true end
...
function M.build_tree(root, max_depth, ignore, max_files_fallback)
  ...
  if git.is_git_repo(root) then
    ...
    if code == 0 and raw and raw ~= '' then
      for file in raw:gmatch('[^\n]+') do
        file = util.trim(file)
        if file ~= '' and not should_ignore(file, ignore) then table.insert(files, file) end
      end
    end
  end
  ...
  if #files == 0 then
    ...
    for name, ftype in vim.fs.dir(root, { depth = (max_depth or 3) + 1 }) do
      if ftype == 'file' then
        ...
        if max_files_fallback and count >= max_files_fallback then break end
      end
    end
  end
  ...
  for _, file in ipairs(files) do
    ...
    if #parts <= (max_depth or 3) + 1 then insert_path(tree, parts) end
  end
