...
local function find_name(node, bufnr)
  ...
  if field and field[1] then return node_text(field[1], bufnr) end

  for child in node:iter_children() do
    if child:named() and name_types[child:type()] then return node_text(child, bufnr) end
  end
...
local function inclusive_end_line(sr, er, ec)
  if ec == 0 and er > sr then return er end
  return er + 1
end
...
function M.current_container(bufnr)
  ...
  if not ok or not parser then return nil, 'No Tree-sitter parser for buffer' end
  ...
  if not trees or not trees[1] then return nil, 'Failed to parse Tree-sitter tree' end
