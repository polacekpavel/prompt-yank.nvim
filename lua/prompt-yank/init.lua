...
local function apply_keymaps()
  ...
  local function parse(value)
    ...
    if type(value) == "table" then
      return { lhs = value.lhs or value[1], format = value.format }
    end
    return nil
  end
  ...
  local function nmap(name, fn, desc)
    ...
    if not km or not km.lhs then
      return
    end
    vim.keymap.set("n", km.lhs, function()
      fn({ format = km.format })
...
function M.yank_selection(opts)
  ...
  yank.copy(text, opts)
  yank.notify(("Copied %d lines from %s"):format(line_end - line_start + 1, ctx.filepath), nil, opts)
  return text
end
...
function M.yank_file(opts)
  ...
  if fullpath then
    ...
    if not yank.ensure_size_ok("file", size, conf.limits.max_file_bytes) then
      return nil
    end
  end
  ...
  local text = yank.format_code_block(ctx, opts)
  yank.copy(text, opts)
  yank.notify(("Copied entire file (%d lines) from %s"):format(total_lines, ctx.filepath), nil, opts)
  return text
end
...
function M.yank_diagnostics(opts)
  ...
  yank.copy(text, opts)
  yank.notify(("Copied %d lines + diagnostics from %s"):format(line_end - line_start + 1, ctx.filepath), nil, opts)
  return text
end
...
function M.yank_diff(opts)
  ...
  if range_start and range_end then
    ...
    if not code then
      code = yank.get_range_text(bufnr, range_start, range_end)
    end
    local ctx = yank.build_ctx_for_buffer(bufnr, code, range_start, range_end)
    if not diff then
      local text = yank.format_code_block(ctx, opts)
      yank.copy(text, opts)
      yank.notify(("No uncommitted changes; copied selection from %s"):format(ctx.filepath), nil, opts)
      return text
    end
...
function M.yank_tree(opts)
  ...
  if selection_start and selection_end then
    ...
    if not rel then
      ...
      local text = yank.format_code_block(ctx, opts)
      yank.copy(text, opts)
      yank.notify("File is outside project root; tree unavailable; copied selection only", vim.log.levels.WARN, opts)
      return text
    end
...
function M.yank_context(opts)
  ...
  local ctx = yank.build_ctx_for_buffer(bufnr, code, start_ctx, end_ctx, { context_lines = context_lines })

  local text = yank.format_named_template("context", ctx, opts)
  yank.copy(text, opts)
  yank.notify(("Copied context (L%d-L%d) from %s"):format(start_ctx, end_ctx, ctx.filepath), nil, opts)
  return text
end
...
function M.yank_remote(opts)
  ...
  if not rel then
    ...
    local text = yank.format_code_block(ctx, opts)
    yank.copy(text, opts)
    yank.notify("File is outside project root; remote URL unavailable; copied code only", vim.log.levels.WARN, opts)
    return text
  end

  local remote_url = base and commit and git.build_remote_url(base, provider, commit, rel, line_start, line_end)
  ...
  local ctx = yank.build_ctx_for_path(fullpath, root, code, lang.for_buffer(bufnr, conf), line_start, line_end, {
...
function M.yank_function(opts)
  ...
  yank.copy(text, opts)
  yank.notify(("Copied %s (%s) from %s"):format(info.node_type, info.name or "unnamed", ctx.filepath), nil, opts)
  return text
end
...
function M.yank_files(paths, opts)
    ...
    if path ~= "" then
      if util.is_sensitive(path, conf.sensitive_patterns) then
        ...
        if util.path_is_absolute(path) then
          ...
          fullpath = (vim.fs and vim.fs.joinpath) and vim.fs.joinpath(root, path) or (root .. "/" .. path)
        end
        ...
        elseif util.is_sensitive(rel, conf.sensitive_patterns) then
          ...
          if code then
            ...
            yank.notify(("Skipped %s: %s"):format(path, err or "unreadable"), vim.log.levels.WARN, opts)
          end
        end
      end
    end
  ...
  if #skipped_sensitive > 0 then
    yank.notify(
      ("Skipped %d sensitive file(s): %s"):format(#skipped_sensitive, table.concat(skipped_sensitive, ", ")),
      vim.log.levels.WARN,
      opts
    )
  end
