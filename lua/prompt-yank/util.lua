...
function M.realpath(path)
  ...
  if not uv or not uv.fs_realpath then
    return nil
  end
...
function M.path_relative(path, root)
  ...
  if root_with_sep:sub(-1) ~= sep then
    root_with_sep = root_with_sep .. sep
  end

  if path:sub(1, #root_with_sep) == root_with_sep then
    return path:sub(#root_with_sep + 1)
  end
  ...
  if sep ~= "/" then
    ...
    if alt_root_sep:sub(-1) ~= "/" then
      alt_root_sep = alt_root_sep .. "/"
    end
...
local function is_parent_relative(rel)
  ...
  if rel:sub(1, 3) == ".." .. sep then
    return true
  end
  if sep ~= "/" and rel:sub(1, 3) == "../" then
    return true
  end
  if sep ~= "\\" and rel:sub(1, 3) == "..\\" then
    return true
...
function M.relpath_under_root(fullpath, root)
  if not fullpath or not root then
    return nil
  end
  ...
  if check_full == "" or check_root == "" then
    return nil
  end
  ...
  if M.path_is_absolute(rel) then
    return nil
  end
  if is_parent_relative(rel) then
    return nil
  end
...
function M.git_root(dir)
  ...
  out = M.trim(out)
  if out == "" or out:match("^fatal:") then
    return nil
  end
  return out
end
...
function M.display_path(fullpath, root, path_style)
  ...
  if path_style == "relative" then
    ...
    if M.path_is_absolute(rel_to_cwd) then
      return vim.fn.fnamemodify(fullpath, ":t")
    end
    return rel_to_cwd
  end
