...
function M.notify(message, level, opts)
  ...
  if opts and opts.notify ~= nil then
    notify = opts.notify
  end
  if not notify then
    return
...
function M.format_code_block(ctx, opts)
  return format.render_code_block(ctx, opts)
end
...
function M.get_visual_selection(bufnr, opts)
  ...
  if in_selection then
    ...
    if not (opts and opts.from_visual) then
      return nil
    end
    pos1 = vim.fn.getpos("'<")
    pos2 = vim.fn.getpos("'>")
  ...
  if end_col_exclusive > #last_line then
